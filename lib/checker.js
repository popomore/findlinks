...
const defaults = {
  ...
  logger: null,
  puppeteer: null,
  // The default timeout of pages to render
  puppeteerRenderTimeout: 30 * 1000,
  attributes: [
    // <a href=""></a>
    [ 'a', 'href' ],
  ],
...
class Checker {
  ...
  constructor(opt) {
    ...
    this.srcUrl = opt.src;
    this.puppeteerRenderTimeout = opt.puppeteerRenderTimeout;
  ...
  async _check(currentUrl, refererUrl) {
    ...
    const { html, isHtml, status } = await this._getHTML(currentUrl);
  ...
  async _getHTML(url) {
    ...
    const isHtml = (ret.headers['content-type'] || '').startsWith('text/html');
    if (!this.browser) {
      return {
        status: ret.status,
        isHtml,
    ...
    const html = await getHTMLFromPuppeteer({
      ...
      puppeteerRenderTimeout: this.puppeteerRenderTimeout,
    });
    return { status: ret.status, isHtml, html };
  }
